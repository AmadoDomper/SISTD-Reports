...
namespace ADSql
{
    public class UsuarioAD
    {
        ...
        public Usuario validarUsuario(string UserName, string Pass)
        {
            ...
            oDatabase.AddInParameter(oDbCommand, "@Logueo", DbType.String, UserName);
            oDatabase.AddInParameter(oDbCommand, "@Contrasenia", DbType.String, Pass);
            Usuario user = null;
            using (IDataReader datos = oDatabase.ExecuteReader(oDbCommand))
            {
                ...
                {
                    user = new Usuario();
                    user.UserName = DataUtil.DbValueToDefault<String>(datos[datos.GetOrdinal("Logueo")]);
                    user.UserId = DataUtil.DbValueToDefault<Int32>(datos[datos.GetOrdinal("UsuarioId")]);
                }
            }
            return user;
        }
